...
public class AbsoluteRotation extends SubsystemBase{
    ...
    public AbsoluteRotation(DoubleSupplier joyRightX, 
        ...
        SmartDashboard.putNumber("rotP", 2);
    ...
    public void periodic() {
        ...
        joyAngle = Math.atan2(-joyRightY.getAsDouble(), joyRightX.getAsDouble()) + (Math.PI / 2);
        ...
        rotSpeed = MathUtil.clamp(controller.calculate(robAngle), -Constants.SwerveConstants.MaxAngularSpeed, Constants.SwerveConstants.MaxAngularSpeed);

        if(Math.sqrt(joyRightY.getAsDouble() * joyRightY.getAsDouble() + joyRightX.getAsDouble() * joyRightX.getAsDouble()) < 0.08) {
            rotSpeed = 0;
        }
